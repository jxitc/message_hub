...
class SmsProcessor(
    ...
    suspend fun processSmsMessage(smsMessage: SmsMessage) {
        try {
            ...
            when (result) {
                is com.jxitc.messagehub.domain.model.SmsProcessingResult.Success -> {
                    Logger.i("SmsProcessor", "SMS processed successfully: ${result.message}")
                }
                is com.jxitc.messagehub.domain.model.SmsProcessingResult.Filtered -> {
                    Logger.d("SmsProcessor", "SMS filtered: ${result.reason}")
                }
                is com.jxitc.messagehub.domain.model.SmsProcessingResult.Failed -> {
