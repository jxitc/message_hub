...
- [x] 3.4.12 **P1 - FIXED: notification dedup race condition** — reported 2026-09-04 (Dedup Issue 1). Identical Clock alarm notification stored 7× in MH. Root cause: `NotificationProcessor` dedup read-compare-write was not atomic, and `NotificationListener` dispatches each notification on its own coroutine — a burst of N near-simultaneous identical notifications passed the gate N times. **FIXED 2026-09-12**: new `domain/service/NotificationDeduplicator.kt` performs the read-compare-write inside `synchronized(lock)`; `NotificationProcessor` delegates to it (semantics unchanged: A A A → 1, A B A → 3). Covered by `NotificationDeduplicatorTest` (4 cases) including `concurrentBurstOnlyOnePasses` (100 concurrent identical notifications → exactly 1 passes). NOTE: not reproducible on-device (needs an app bursting identical notifications; the adb debug-broadcast backdoor has its extras stripped by ColorOS on Android 16), so the guarantee rests on the unit test + the determinism of `synchronized`.
...
- ~~**Dedup Issue 1** (3.4.12): Notification consecutive-dedup defeated by race condition~~ **FIXED 2026-09-12** ✅ — identical Clock alarm notification landed 7× (7 message ids, same content/time) because the dedup read-compare-write was not atomic while each notification ran on its own coroutine. Now `NotificationDeduplicator` synchronizes the check+update; `NotificationDeduplicatorTest.concurrentBurstOnlyOnePasses` (100 concurrent identical notifications → 1 stored) guards it.
- **Log Issue 1** (1.2.7 family): **Launch crash with no Java stack trace** — reported 2026-09-12: app opened, showed cached messages, closed ~5s later with no dialog. Root cause was NOT a Java exception: `HttpLoggingInterceptor.Level.BODY` logged an entire response body (5508 bytes) as one line, exceeding logd's ~4068-byte single-entry limit, and Android 16's log-chunking path aborts the process via an ubsan sub-overflow (SIGABRT). **FIXED 2026-09-12**: logging level BASIC + `Logger` clamps every line to 3500 UTF-8 bytes; verified on device with a 48KB response.
