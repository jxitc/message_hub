...
- [ ] 3.4.12 **P1 - FIX notification dedup race condition** — reported 2026-09-04 (Dedup Issue 1). Identical Clock alarm notification stored 7× in MH. Root cause: `NotificationProcessor` dedup read-compare-write on `lastNotification` is not atomic, and `NotificationListener` dispatches each notification on its own coroutine — a burst of N near-simultaneous identical notifications passes the gate N times. Fix: wrap the dedup check+update in `synchronized(lock)` (or `Mutex`) so it's atomic; keep the strict-adjacent + "A B A → 3" semantics. Verify by bursting identical notifications → only 1 stored. Files: `domain/service/NotificationProcessor.kt`, `service/NotificationListener.kt`.
...
- **Dedup Issue 1** (3.4.x): **Notification consecutive-dedup defeated by race condition** — reported 2026-09-04: identical Clock alarm notification ("07:30 的闹钟即将响铃", same postTime ~07:15) landed **7×** in MH (7 distinct message ids, same content/time). Root cause: `NotificationProcessor.processNotification` checks+updates `lastNotification` (read, compare, write) WITHOUT synchronization, while `NotificationListener.onNotificationPosted` dispatches each notification on its own coroutine — when an app bursts N near-simultaneous notifications (alarm rings), N coroutines interleave: each reads the still-old `lastNotification` and all pass the dedup gate. Fix scope (task 3.4.12): wrap the read-compare-write in `synchronized(lock)` (or a Mutex) so it is atomic; verify by sending a burst of identical notifications and confirming only 1 is stored. Files: `domain/service/NotificationProcessor.kt` (dedup), `service/NotificationListener.kt` (dispatch context). Needs build+test on the Android dev machine (this repo has no Android toolchain).
