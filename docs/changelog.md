...
## 2026-09-20（幂等入库 + 清掉 1.4 万条重复 + 手机端根因）

- **发现问题**：库里 16,160 条消息里，**14,760 条是同一个事件的重复拷贝** —— 一台
  `OPPO-Find-X7-db1792` 只有 **991 个**真实事件（+336 Evernote +77 邮件 +1 网页）。
  其中最夸张的一个 Chrome 通知被存了 **271 次**（10 秒内，06:40:12.719 → 06:40:23.007）。
- **根因（在手机端，不在服务端）**：`ProcessNotificationUseCase` / `ProcessSmsUseCase` 每收到
  一条通知/短信就调一次 `syncPendingMemories()`——那是**全量**扫描，而且各自一个协程毫无互斥。
  一次通知风暴里 56 个通知 → 56 个并发全量扫描，都从队头走一遍，每条再重试 5 次：
  **56 × 5 = 280，实测 281**。数据里同一 key 的到达时间**成簇**出现（不是轮转），
  正是"多个会话按同一顺序走同一张表"的指纹，这才把机制定下来。
- **服务端防线（本次实现）**：`message_identity.py` 定义"什么算同一条消息"，入库与清理共用一份规则：
  - 推送 `v1|ntf|<package_name>|<notification_id>|<timestamp>`（抖音所有通知共用同一个
    `notification_id`，真正区分它们的是 timestamp，缺一不可）
  - 有渠道 id 的 `v1|id|<type>|<message_id>`（短信/邮件/通话记录）
  - 无渠道 id 的短信类 `v1|txt|<type>|<sender>|<timestamp>|sha1(正文)`
  - **手写 NOTE/DOCUMENT 没有键，永不判重**（两条一模一样的笔记是两件用户想留的事）
  - `messages.natural_key` + **唯一索引**（唯一索引才是保证：应用层"先查再插"挡不住并发）
  - 重复上报 → **HTTP 200** `{"duplicate": true, "id": <已有 id>}`（不是 409：重试中的客户端
    必须把它读成成功才会停止重试）；判定发生在**附件落盘之前**，重传只花一次 SELECT
  - 唯一的例外：重复上报的正文**比已存的更长**就替换正文（通知第一次抓到"视频标题加载失败"、
    重试才拿到真标题；只留第一份就把唯一有用的那份丢了），其余字段一律不动
- **清理**：`scripts/dedup-messages.py`（默认演练）。同一组留**信息量最大**的那份
  （正文最长 → 最早 → id 最小）；`--report` 会额外列出**组内正文不一致**的组，把每个变体都打出来
  并标注保留哪一份——「最长」是启发式，这份清单是唯一能看出留错的地方。
  线上：**16,160 → 1,400 条**（删 14,760，回填 1,057 个键，343 条无键原样保留，残留重复组 0）。
  删前先 `sqlite3 backup()` 存了 `message_hub.db.bak-20260920-pre-dedup`（16,160 行）。
  5 个正文不一致的组**全部留住了带真标题的那份**。
- **没做的（有证据说明为什么）**：剩下 919 条通知里，有 38~56 条是**同一通知被手机在几毫秒内
  抓了多次**（手机自己给的 `metadata.timestamp` 相差 1~40ms），按时间窗合并能省下它们，
  但**时间窗会误伤真实数据**：`android/nid=38` 里两条 Telegram 登录码相差 **1ms** 却是两个账号
  （1794874969 / 6224730393），`com.yusi.chongchong/nid=412` 里两首不同的歌相差 **107ms**。
  所以键保持精确，「加载失败」占位行留在库里——宁可留下噪声，也不能合并掉真实事件。
- **手机端修复**（`MemorySyncService`）：① 收到一条消息只上传**那一条**（`syncMemory(id)`），
  不再触发全量扫描；② 上传**永不并发**，全量扫描用 `tryLock`，正在跑就**直接丢弃**而不是排队。
  依赖改为函数注入（与 `AttachmentExtractionPoller` 同一手法），所以并发规则能在 JVM 单测里跑。
  新增 9 条回归测试；**做过反向验证**：把实现退回旧行为（排队 + 不串行）后，
  "uploads never run at the same time" 与 "a sweep already running is not started a second time"
  两条**确实失败**——测试抓得住这次事故。
  顺带：`app/build.gradle.kts` 加 `testOptions.unitTests.isReturnDefaultValues = true`
  （否则 `android.util.Log` 在单测里一调用就抛，任何走 Logger 的逻辑都没法测）。
- **验证**：服务端 pytest **150 例全过**（新增 `tests/test_idempotency.py` 24 条）；
  Android 单测 **161 例全过**。线上自检（`/tmp/selftest_prod.py`）：首次 201、连发 3 次全
  200 `duplicate=true` 且 id 相同、不同通知仍 201、自检数据已清干净。
