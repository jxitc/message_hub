...
def test_to_header_is_the_recipient():
    ...
    # Delivered-To is kept for the record but must not be mistaken for the target.
...
def test_x_original_to_wins_for_the_record():
    parsed = mc.parse_message(raw(
        'From: a@b.com\n'
...
def test_payload_puts_to_in_content_and_metadata():
    parsed = mc.parse_message(raw(
    ...
    payload = mc.build_payload(parsed, 'main')
    assert '\nTo: Xiao J <jxitc@hotmail.com>\n' in payload['content']
    assert payload['metadata']['recipients'] == ['jxitc@hotmail.com']
    assert payload['metadata']['to'] == 'Xiao J <jxitc@hotmail.com>'
    # mailbox/message_id/subject bookkeeping is still there
    assert payload['metadata']['mailbox'] == 'main'
    assert payload['source_device_id'] == 'mail-main'


def test_empty_recipient_values_are_omitted_from_metadata():
    """Keeps the JSON small — no keys with empty strings."""
    parsed = mc.parse_message(raw('From: a@b.com\nTo: one@example.com\nSubject: hi'))
    metadata = mc.build_payload(parsed, 'main')['metadata']
    assert 'cc' not in metadata
    assert 'delivered_to' not in metadata
    assert 'original_to' not in metadata
